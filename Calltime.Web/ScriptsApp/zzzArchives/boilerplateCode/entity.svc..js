(function () {
    ...
    ProductionSvc.$inject = ['crudCalltimeWebApiSvc'];
    
    function ProductionSvc(crudCalltimeWebApiSvc) {
        var svc = this;
        ...
        function _getSuccess(response) {
            if (response.data.items && response.data.items.length > 0) {
                svc.model.entityList = response.data.items;
                svc.model.keys = Object.keys(response.data.items[0]);
            }
